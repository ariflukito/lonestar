...
# $Header: /var/cvsroot/gentoo-x86/mail-client/evolution/evolution-2.8.1-r1.ebuild,v 1.1 2006/10/13 21:06:47 dang Exp $
...
src_unpack() {
	...
	# Fix filter-bar crash
	epatch ${FILESDIR}/${P}-filter-bar-crash.patch

	# Add bogofilter junk plugin source
	use bogofilter && epatch ${FILESDIR}/${PN}-2.7.3-bf-junk.patch.gz

	epatch ${FILESDIR}/${PN}-2.6.2-mailicon.patch
...
src_install() {
	...
	insinto /usr/share/${PN}/2.8/images
	doins ${FILESDIR}/*.png
}
